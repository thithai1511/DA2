...
export default function LiveBoard() {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      <h3 className="font-bold mb-2">Su kien live</h3>
      <ul className="space-y-2">
        {events.map((ev, i) => (
          <li key={i} className="border rounded p-2">
            <div className="text-sm">Loai: {ev.type}</div>
            <div className="text-sm">SV: {ev.sv?.ho_ten} ({ev.sv?.mssv})</div>
            <div className="text-sm">Lop: {ev.lop?.ten_lop}</div>
            <div className="text-sm">Thoi gian: {ev.time}</div>
          </li>
        ))}
      </ul>
    </div>
